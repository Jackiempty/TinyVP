...
void RTLBackend::vadd(const int32_t* vec_a, const int32_t* vec_b, int32_t* vec_c, uint32_t vec_size) {
  ...
  while (!shm_h2a.isFlagSet(READY_BIT)) { std::this_thread::sleep_for(std::chrono::milliseconds(1)); }
  ...
  shm_h2a.setFlag(VALID_BIT);

  // Wait for Accelerator ACK
  while (!shm_h2a.isFlagSet(ACK_BIT)) { std::this_thread::sleep_for(std::chrono::milliseconds(1)); }

  // Clear flags to complete transaction
  shm_h2a.clearFlag(ACK_BIT);
  shm_h2a.clearFlag(VALID_BIT);
  ...
  shm_a2h.setFlag(READY_BIT);

  // Wait for Accelerator VALID
  while (!shm_a2h.isFlagSet(VALID_BIT)) { std::this_thread::sleep_for(std::chrono::milliseconds(1)); }

  // Clear READY during data transfer
  shm_a2h.clearFlag(READY_BIT);

  // Deserialize result payload
  getVectorData(resp_ptr, vec_c, 0, vec_size);

  // Assert ACK to acknowledge receipt
  shm_a2h.setFlag(ACK_BIT);
